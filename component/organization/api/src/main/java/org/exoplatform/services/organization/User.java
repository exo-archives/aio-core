 ...
 * Time: 3:22:54 PM
 *
 * This is the interface for a User data model. The OrganizationService implementor can use the different 
 * strategy to implement this class,  he can use the native field for each get method or use a Map to hold
 * the user data. Note that after each set method is called. The developer need to call  
 * @see UserHandler saveUser(..) to persist the change
 */
public interface User {
  /**
   * This method  should return the username of  the user.  The username should be unique and the user database
   * should not have 2 user record with the same username
   * @return
   */
   ...
   * 
   * TODO:  I think we should not have this method. the username should be set only for the first time. So 
   * we can pass the username to the @see UserHandler createUserInstance() method.
   */
  ...
  /**
   * @return  return the full name of the user.  The full name shoul have the format: first name, last name by
   * default
   */
