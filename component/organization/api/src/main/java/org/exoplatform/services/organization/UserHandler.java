 ...
 * Oct 13, 2005
 * 
 * This class  is acted as a sub component of the organization service. It is used to manage the user  
 * account and broadcast  the user event to all the registered listener in the organization service. 
 * The user event can be: new user event,  update user event and delete  user event. Each event should 
 * have 2 phases:  pre event and post event. The method createUser , saveUser and removeUser broadcast 
...
public interface UserHandler {
   ...
   * @param broadcast:  If the broadcast value  is  true ,  then the UserHandler should  broadcast
   * the event  to  all the listener that  register with the organization service. For example, the portal
   * service register an  user event listener  with  the organization service. when a new account 
   * is created, a  portal configuration should be created  for  the new  user account at the same
   * time. In this case  the portal user event listener will  be called in the createUser method.  
   * @throws Exception:  The exception can be throwed if  the the UserHandler cannot persist the user object
   * or any listeners  fail  to handle the user event.
   */
  public void createUser(User user, boolean broadcast) throws Exception ;
  /**
   * This method  is used to  update  an existing User  object
   * @param user  The user object to  update
   * @param broadcast If the broadcast is  true ,  then all the user event listener that register with the
   * organization service will be called
   * @throws Exception The exception can be throwed if  the the UserHandler cannot save the user object
   * or any listeners  fail  to handle the user event.
   */
  public void saveUser(User user, boolean broadcast) throws Exception ;
  /**
   * Remove an user  and broadcast the event to all the registerd  listener. When the user
   * is removed , the user profile and all the membershisp of the user should be removed as well. 
   * @param userName The user should be removed  from the user database
   * @param broadcast If broadcast is true,  the the  delete user event  should be broadcasted to all registered
   * listener
   ...
   * @return The method return null if there no user matchs the given username. The method return an User 
   * object if an user that mathch the username. 
   * @throws Exception The exception is throwed if the method fail to access the user database or more than
   ...
   * @param groupId id  of the group. The return users list   should be in this  group
   ...
   * @return return a page list  iterator. The page list should allow the developer get all
   * the users  or get a page of  users  if the return number of users  is too large.
   * @throws Exception
   */
   ...
   * @param username
   * @param password
   * @return  return true  if the username and the password is match  with an user record 
   * in the database, else  return false.
