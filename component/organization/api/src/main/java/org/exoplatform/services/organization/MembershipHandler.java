 ...
 * membership - the ralation of user ,  group, and membership type - and broadcast  the membership 
 * event to all the registered listener in the organization service. The membership  event can be: new linked membership  and delete 
 * the membership type event. Each event should have 2 phases:  pre event and post event. 
 * The method linkMembership(..) and  removeMembership(..) broadcast the event at each phase so 
...
public interface MembershipHandler {
   ...
   * @param broadcast Broadcast the event if the value of  the broadcast is 'true'
   * @throws Exception An exception is throwed if the method is fail to access the database, a membership
   * record with the same user , group and membership type exisited or any listener fail to handle the
   * event.
   */
   ...
   * @param broadcast Broadcast the event to the registered listeners if the broadcast event is 'true'
   ...
   * @throws Exception An exception is throwed if the method fail to access the database or no membership 
   * is found.
   */
   ...
   * @throws Exception Usually an exception is thrown if the method cannot access the database
   */
   ...
   * @throws Exception Usually an exception is throwed if the method cannot access the database.
   */
  public Collection findMembershipsByUser(String userName) throws Exception ;
  /**
   * Use this method  to find all the membership in a group. Note that an user can have more than
   * one membership in a  group. For example  , user admin can have meberhsip 'member' and 'admin'
   * in the  group '/users'
   * @param group 
