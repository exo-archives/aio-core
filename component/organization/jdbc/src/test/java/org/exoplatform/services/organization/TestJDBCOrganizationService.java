...
public class TestJDBCOrganizationService extends BasicTestCase {
  ...
  public TestJDBCOrganizationService(String s) {
    super(s);
  }
  ...
  public void setUp() throws Exception {
    ...
    //service_ = new OrganizationServiceImpl(listenerService, dataSource);
    service_ = 
      (OrganizationService) manager.getComponentInstanceOfType(OrganizationService.class);
//    System.out.println( "\n\n\n\n Organization service : " + service_ + "\n\n\n") ;
  ...
  protected String getDescription() {
    if(!runtest)  return "" ;
    return "\n\n************** Test JDBC organization service ****************\n\n";
  }
  

  public User createUser(String userName) throws Exception {   
    User user = userHandler_.createUserInstance(userName) ;
    user.setPassword("passDefault") ;
    user.setFirstName("FirstNameDefault") ;
    user.setLastName("LastNamedefault") ;
...
//    String thuanName = "thuannd" ;   
//    String lxName = "lxchiati";
//   //Test createUser()
//    User thuanUser = createUser(thuanName) ;
//    thuanUser.setEmail("thuannd@yahoo.com");
//    userHandler_.createUser(thuanUser, true);
//   
//    User lxuser = createUser(lxName);
//    lxuser.setEmail("thuannd@yahoo.com");
//    userHandler_.createUser(lxuser, true);
//    User savedUser = userHandler_.findUserByName(thuanName);
//    assertEquals("Expect user name is: ", thuanName, savedUser.getUserName());
//    User lxUser = userHandler_.findUserByName(lxName);
//    assertEquals("Expect user name is: ", lxName, lxUser.getUserName());
//    userHandler_.removeUser(lxName, true);
//    savedUser = userHandler_.findUserByName(lxName);
//    if(savedUser != null) assertNotSame("Expect user name is: ", "asdfasdfasdf", savedUser.getUserName());
//    
//    assertTrue(userHandler_.authenticate(thuanUser.getUserName(), thuanUser.getPassword()));
//    assertFalse(userHandler_.authenticate(thuanUser.getUserName(), thuanUser.getPassword()+ "abc"));
//    thuanUser.setPassword(thuanUser.getPassword()+"abc");
//    userHandler_.saveUser(thuanUser, true);
//    assertTrue(userHandler_.authenticate(thuanUser.getUserName(), thuanUser.getPassword()));
//    
//    userHandler_.createUser(lxuser, true);
//    Query query = new Query();
//    query.setEmail("thuannd@yahoo.com");
//    PageList pageList = userHandler_.findUsers(query);
//    assertTrue (pageList.getAvailable() == 2) ;
//    
//    /* Update user's information */
//    thuanUser.setFirstName("Exo(Update)");
//    userHandler_.saveUser(thuanUser, false);
//    UserProfile up = profileHandler_.findUserProfileByName(thuanName);
//    up.getUserInfoMap().put("user.gender", "male");
//    
//    System.out.println("\n\nExecute: 123 value=" + up.getUserInfoMap().get("user.gender"));
//    profileHandler_.saveUserProfile(up, true);
//    up = profileHandler_.findUserProfileByName(thuanName);
//    assertEquals("expect first name is", "Exo(Update)", thuanUser.getFirstName());
...
////      System.out.println("come here: "  + piterator.currentPage().size());
////      assertTrue (piterator.currentPage().size() == 3) ;
////    } catch (Exception e) {
////      e.printStackTrace();
////    }
//    
...
//     */
//    userHandler_.removeUser(thuanUser.getUserName(), true);
//    assertEquals("User: USER is removed: ", null, userHandler_.findUserByName(thuanUser.getUserName()));
//    System.out.println("\n      execure here");
//    assertTrue(" user's profile of USER was removed:",profileHandler_.findUserProfileByName(thuanName)==null) ;
//  }
  ...
  public void testGroup() throws Exception {    
    ...
    System.out.println("Tao them mot group");
    ...
    System.out.println("Test find child");
    ...
    System.out.println("Test remove group");
  }
...
//
  public void testMembership() throws Exception {    
    ...
    System.out.println( " =======Size: " + mems.size());
