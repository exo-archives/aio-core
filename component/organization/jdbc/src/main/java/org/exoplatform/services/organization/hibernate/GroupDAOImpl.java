...
public class GroupDAOImpl implements GroupHandler { 
  ...
  public void addChild(Group parent, Group child, boolean  broadcast) throws Exception {
    ...
    if(broadcast)  preSave(child, true)  ;
    childImpl.setId(groupId);
    session.save(childImpl);
    if(broadcast) postSave(child, true)  ;
    session.flush() ;
  }

  public void saveGroup(Group group, boolean broadcast) throws Exception {
  	Session session = service_.openSession();    
    if(broadcast) preSave(group, false)  ;    
  	session.update(group);
    if(broadcast) postSave(group, false)  ;   
    session.flush();    
  }
  ...
  public Group removeGroup(Group group, boolean broadcast) throws Exception {
    ...
    for(int i=0; i<entries.size(); i++) removeGroup( (Group) entries.get(i),  broadcast);   
  ...
  public Collection findGroupByMembership(String userName, String membershipType) throws Exception {
    ...
    Query q  = session.createQuery(queryFindGroupByMembership).
                       setString(0, userName).setString(1, membershipType) ;
  	List groups =  q.list();
  	return groups ;
  }
  ...
  public Collection findGroups(Group parent) throws Exception {
  	Session session = service_.openSession();
  	if (parent == null) return session.createQuery(queryFindRootGroups).list();
