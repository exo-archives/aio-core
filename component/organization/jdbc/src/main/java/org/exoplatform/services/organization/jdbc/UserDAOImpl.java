...
public class UserDAOImpl extends StandardSQLDAO<UserImpl> implements UserHandler {
  ...
  public PageList findUsers(org.exoplatform.services.organization.Query orgQuery) throws Exception {
   ...
   if (orgQuery.getUserName()!=null) dbQuery.addLIKE("UPPER(USER_NAME)", orgQuery.getUserName().toUpperCase());
    if(orgQuery.getFirstName()!=null) dbQuery.addLIKE("UPPER(FIRST_NAME)", orgQuery.getFirstName().toUpperCase());
   if (orgQuery.getLastName()!=null) dbQuery.addLIKE("UPPER(LAST_NAME)", orgQuery.getLastName().toUpperCase());
