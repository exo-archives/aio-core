...
//  static  void createMembershipEntries(Collection c, Session session) throws Exception {
...
//    User user =(User)service_.findExactOne(session,UserHandlerImpl.queryFindUserByName, userName);
    membership.setUserName(user.getUserName());
    membership.setMembershipType( mt.getName()) ;    
    membership.setGroupId( g.getId());    
  	if( membership.getId() != null)  throw new Exception(" Membership id isn't null!");
    if( findMembershipByUserGroupAndType( user.getUserName(), g.getId(), mt.getName()) != null) return;   
  	String id = IdentifierUtil.generateUUID(membership) ;
    if(broadcast) preSave(membership, true)  ;
    membership.setId(id);
  	session.save(membership);
    if(broadcast) postSave(membership, true)  ;
  	session.flush();
  }

  public void saveMembership(Membership m, boolean broadcast) throws Exception {
  	Session session = service_.openSession();
    if(broadcast) preSave(m, false)  ;
  	session.update(m);
    if(broadcast) postSave(m, false)  ;
  	session.flush();
  }
...
//    Type[] types = new Type[] { Hibernate.STRING, Hibernate.STRING, Hibernate.STRING };
    List memberships = session.createQuery(queryFindMembershipByUserGroupAndType).
                               setString(0, userName).setString(1, groupId).setString(2,type).list();
...
//  	Type[] types = new Type[] { Hibernate.STRING, Hibernate.STRING };
  	List memberships = session.createQuery(queryFindMembershipsByUserAndGroup).
                               setString(0, userName).setString(1, groupId).list();
  	return memberships;
  }
  ...
  public Collection findMembershipsByGroup(Group group) throws Exception {   
  	Session session = service_.openSession();
  	List memberships = 
      session.createQuery( queryFindMembershipsByGroup).setString(0, group.getId()).list();
  	return memberships;
  }
  
  public Collection findMembershipsByGroupId(String groupId) throws Exception {
    Session session = service_.openSession();
//    List memberships = session.find( queryFindMembershipsByGroup, groupId, Hibernate.STRING );
