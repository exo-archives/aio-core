...
/**
 * The PAM bridging class. Most of the work is done here. <p/> To see debugging
 * output for this class and native code, set the installed logging toolkit
 * level for this class to DEBUG or equivalent. The debugging output for the
 * native code will be sent to <code>STDOUT</code>. <p/> This class may be
 * called directly, or by using JAAS, via the
 * {@link org.exoplatform.services.organization.auth.pam.jaas.JpamLoginModule}.
 * 
...
public class Pam {
  ...
  /**
   * Creates a new PAM object configured with the specified service name. <p/> A
   * file with the same name must exist in /etc/pam.d
   * 
   * @param serviceName
  ...
  /**
   * Sames as <code>authenticateSuccessful</code>, except a
   ...
   * synchronized on the class. The underlying PAM mechanism is not threadsafe.
   * 
